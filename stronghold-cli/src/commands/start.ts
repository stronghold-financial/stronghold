...
import { StrongholdCommand, SIGNALS } from '../command'
import { StrongholdNode, NodeUtils, PromiseUtils } from 'stronghold'
import {
  ConfigFlag,
  ConfigFlagKey,
  DatabaseFlag,
...
export default class Start extends StrongholdCommand {
    ...
    if (!trees.valid) {
      this.log(
        `Error starting node: your merkle trees are corrupt: ${String(trees.reason)}.` +
          `\n  1. Run stronghold chain:repair to attempt repair` +
   ...
   */
  async firstRun(node: StrongholdNode): Promise<void> {
