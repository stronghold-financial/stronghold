...
import { DEFAULT_CONFIG_NAME, JSONUtils } from 'stronghold'
...
export class EditCommand extends StrongholdCommand {
  static description = `Edit the config in your configured editor

  Set the editor in either EDITOR environment variable, or set 'editor' in your stronghold config`
  ...
  async start(): Promise<void> {
    ...
    const folderPath = await mkdtempAsync(path.join(tmpDir, 'stronghold'))
