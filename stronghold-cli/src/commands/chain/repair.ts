...
export default class RepairChain extends StrongholdCommand {
  ...
  async repairChain(node: StrongholdNode, speed: Meter, progress: ProgressBar): Promise<void> {
    Assert.isNotNull(node.chain.head)
    ...
    const total = Number(node.chain.head.sequence)
    let done = 0
    let head = node.chain.head as StrongholdBlockHeader | null
