...
export const DEFAULT_GET_FUNDS_API = 'https://api.stronghold.financial/api/v1/getFunds'
export const DEFAULT_TELEMETRY_API = 'https://api.stronghold.financial/api/v1/writeTelemetry'
export const DEFAULT_BOOTSTRAP_NODE = 'test.bn1.stronghold.financial'
...
export class Config extends KeyStore<ConfigOptions> {
  ...
  static GetDefaults(files: FileSystem, dataDir: string): ConfigOptions {
    return {
      ...
      ipcPath: files.resolve(files.join(dataDir || DEFAULT_DATA_DIR, 'stronghold.ipc')),
      isWorker: false,
      logLevel: '*:info',
      logPrefix: '',
      miningForce: false,
