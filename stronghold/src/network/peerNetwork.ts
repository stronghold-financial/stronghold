...
import { StrongholdBlock, StrongholdBlockSerialized } from '../primitives/block'
import {
...
import { BlockHash, StrongholdBlockHeader } from '../primitives/blockheader'
...
export class PeerNetwork {
  ...
  private readonly node: StrongholdNode
  private readonly strategy: StrongholdStrategy
  private readonly chain: StrongholdBlockchain
  ...
  gossipBlock(block: StrongholdBlock): void {
  ...
  start(): void {
    ...
    if (this.listen && 'Server' in this.localPeer.webSocket && this.localPeer.port != null) {
      ...
      this.onIsReadyChanged.on((isReady: boolean) => {
        if (isReady) {
          this.logger.info(`Connected to the Stronghold network`)
          this.node.onPeerNetworkReady()
        } else {
          this.logger.info(`Not connected to the Stronghold network`)
    ...
    for (const node of this.bootstrapNodes) {
      ...
      // it's running on the default Stronghold websocket port
  ...
  async getBlocks(
    ...
    start: Buffer | bigint,
    limit: number,
  ): Promise<StrongholdBlockSerialized[]> {
    const origin = start instanceof Buffer ? start.toString('hex') : Number(start)
  ...
  private async resolveSequenceOrHash(
    start: string | number,
  ): Promise<StrongholdBlockHeader | null> {
