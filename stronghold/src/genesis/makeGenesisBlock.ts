...
import { generateKey, WasmNote, WasmTransaction } from 'stronghold-wasm-nodejs'
...
export async function makeGenesisBlock(
  ...
  const postedInitialTransaction = new StrongholdTransaction(
  ...
  const postedTransaction = new StrongholdTransaction(
  ...
  const postedMinersFeeTransaction = new StrongholdTransaction(
