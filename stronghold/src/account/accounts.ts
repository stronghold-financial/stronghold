...
import { generateKey, generateNewPublicAddress } from 'stronghold-wasm-nodejs'
...
import { StrongholdWitness } from '../merkletree/witness'
import { StrongholdMemPool } from '../memPool'
...
export class Accounts {
  ...
  readonly onBroadcastTransaction = new Event<[transaction: StrongholdTransaction]>()
  ...
  protected readonly transactionMap = new BufferMap<
    Readonly<{
      transaction: StrongholdTransaction
  ...
  protected readonly chain: StrongholdBlockchain
  ...
  async updateHead(): Promise<void> {
    ...
    try {
      const addBlock = async (header: StrongholdBlockHeader): Promise<void> => {
        this.logger.debug(
      ...
      const removeBlock = async (header: StrongholdBlockHeader): Promise<void> => {
        this.logger.debug(
  ...
  get shouldRescan(): boolean {
    ...
    for (const account of this.accounts.values()) {
      if (account.rescan !== null) return true
    }
  ...
  private getUnspentNotes(
    account: Account,
  ): ReadonlyArray<{ hash: string; note: StrongholdNote; index: number | null }> {
    const unspentNotes = []
  ...
  getBalance(account: Account): { unconfirmedBalance: BigInt; confirmedBalance: BigInt } {
    ...
    for (const note of notes) {
      ...
      unconfirmedBalance += BigInt(value)
      if (note.index != null) {
        confirmedBalance += BigInt(value)
      }
    }
  ...
  async createTransaction(
    ...
    const notesToSpend: Array<{ note: StrongholdNote; witness: StrongholdWitness }> = []
    ...
    for (const unspentNote of unspentNotes) {
      ...
      if (unspentNote.note.value() > BigInt(0)) {
        ...
        amountNeeded -= BigInt(unspentNote.note.value())
      }
  ...
  broadcastTransaction(transaction: StrongholdTransaction): void {
  ...
  async rebroadcastTransactions(): Promise<void> {
    ...
    for (const [transactionHash, tx] of this.transactionMap) {
      ...
      if (headSequence - submittedSequence < REBROADCAST_SEQUENCE_DELTA) continue
  ...
  async setDefaultAccount(name: string | null): Promise<void> {
    if (this.defaultAccount === name) return
