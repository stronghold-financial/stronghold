...
export class Syncer {
  ...
  readonly chain: StrongholdBlockchain
  readonly strategy: StrongholdStrategy
  ...
  onGossip = new Event<[StrongholdBlock]>()
  ...
  async findAncestor(
    ...
    const hasHash = async (
      hash: Buffer | null,
    ): Promise<{ found: boolean; local: StrongholdBlockHeader | null }> => {
      if (hash == null) {
  ...
  async syncBlocks(peer: Peer, head: Buffer | null, sequence: number): Promise<void> {
    ...
    while (head) {
      ...
      const [
        headBlock,
        ...blocks
      ]: StrongholdBlockSerialized[] = await this.peerNetwork.getBlocks(
        peer,
        head,
  ...
  async addBlock(
    peer: Peer,
    serialized: StrongholdBlockSerialized,
  ): Promise<{
    added: boolean
    block: StrongholdBlock
  ...
  async addNewBlock(peer: Peer, newBlock: StrongholdBlockSerialized): Promise<boolean> {
