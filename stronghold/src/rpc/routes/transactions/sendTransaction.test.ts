  ...
  it('throws if not connected to network', async () => {
    try {
      ...
      expect(e.message).toContain(
        'Your node must be connected to the Stronghold network to send a transaction',
      )
    }
  })
  ...
  describe('Connected to the network', () => {
    it('throws if the chain is outdated', async () => {
      ...
      try {
        ...
        if (!(e instanceof ResponseError)) throw e
        expect(e.message).toContain(
          'Your node must be synced with the Stronghold network to send a transaction. Please try again later',
        )
      }
    })
