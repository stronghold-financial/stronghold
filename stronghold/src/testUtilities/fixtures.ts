...
import { generateKey } from 'stronghold-wasm-nodejs'
...
export async function useBlockFixture(
  ...
  generate: FixtureGenerate<StrongholdBlock>,
  addTransactionsTo?: Accounts,
): Promise<StrongholdBlock> {
  return useFixture(generate, {
    process: async (block: StrongholdBlock): Promise<void> => {
      if (addTransactionsTo) {
        await restoreBlockFixtureToAccounts(block, addTransactionsTo)
      }
    },
    serialize: (block: StrongholdBlock): StrongholdBlockSerialized => {
      return chain.strategy.blockSerde.serialize(block)
    },
    deserialize: (serialized: StrongholdBlockSerialized): StrongholdBlock => {
