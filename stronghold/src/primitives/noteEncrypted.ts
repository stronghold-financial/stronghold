...
import { WasmNoteEncrypted } from 'stronghold-wasm-nodejs'
import Serde from '../serde'
import { StrongholdNote } from './note'
...
export class StrongholdNoteEncrypted {
  ...
  decryptNoteForOwner(ownerHexKey: string): StrongholdNote | undefined {
    const note = this.takeReference().decryptNoteForOwner(ownerHexKey)
    this.returnReference()
    if (note) {
      const serializedNote = note.serialize()
      note.free()
      return new StrongholdNote(Buffer.from(serializedNote))
    }
  }

  decryptNoteForSpender(spenderHexKey: string): StrongholdNote | undefined {
...
export class WasmNoteEncryptedSerde
  implements Serde<StrongholdNoteEncrypted, SerializedWasmNoteEncrypted> {
  equals(note1: StrongholdNoteEncrypted, note2: StrongholdNoteEncrypted): boolean {
    return note1.serialize().equals(note2.serialize())
  }

  serialize(note: StrongholdNoteEncrypted): SerializedWasmNoteEncrypted {
    return note.serialize()
  }

  deserialize(serializedNote: SerializedWasmNoteEncrypted): StrongholdNoteEncrypted {
